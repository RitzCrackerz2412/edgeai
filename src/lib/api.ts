...
async function fetchEspnGameByEventId(leagueName: string, eventId: string): Promise<RawGame | null> {
  ...
  try {
    ...
    return {
      ...
      venue: comp.venue?.fullName ?? data.gameInfo?.venue?.fullName ?? 'Unknown Venue',
      venueId: comp.venue?.id ?? data.gameInfo?.venue?.id ?? 'unknown',
      venueCity: comp.venue?.address?.city ?? data.gameInfo?.venue?.address?.city ?? '',
      venueState: comp.venue?.address?.state ?? data.gameInfo?.venue?.address?.state ?? '',
      venueCountry: comp.venue?.address?.country ?? data.gameInfo?.venue?.address?.country ?? 'USA',
